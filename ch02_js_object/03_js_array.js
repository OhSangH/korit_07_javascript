let fruits = ["사과", "바나나", "키위"];
//toString()
console.log(fruits.toString());

//join()
console.log(fruits.join("/"));

// pop()
console.log(fruits.pop());
console.log(fruits);

// shift()
let firstFruit = fruits.shift();
console.log(firstFruit);
console.log(fruits);

// unshift()
console.log(fruits.unshift("레몬"));
console.log(fruits);

// splice()
let fruits2 = ["Banana", "Orange", "Apple", "Mango"];
fruits2.splice(2, 0, "Lemon");
console.log(fruits2);

//concat()
let arr1 = ["김일", "김이"];
let arr2 = ["김삼", "김사"];
let arr3 = ["김오", "김육"];

let myStudent = arr1.concat(arr2, arr3);
console.log(myStudent);

// slice()
let students = myStudent.slice(2);
console.log(students);

let students2 = myStudent.slice(3, 4);
console.log(students2);

// filter()
/*
  filter() 메서드를 활용하여 words 배열 내에 있는
  단어의 길이가 5 이상인 애들만 추출해서 새로운 배열을 만들어 console에 출력
*/

let words = [
  "부산광역시",
  "부산시",
  "서울 특별시",
  "서울",
  "경상남도 남해시",
  "남해시",
  "광주 광역시",
  "경기도 광주시",
  "오사카부 오사카시",
  "오사카",
];

let result = words.filter(function (word) {
  return word.length > 4;
});
console.log(result.toString());

/*
  점수가 80점 이상인 사람만 가지고 있는 새로운 배열을 작성하는 예시
*/
let persons = [
  {
    name: "김일",
    score: 76,
    city: "서울",
  },
  {
    name: "김이",
    score: 98,
    city: "대전",
  },
  {
    name: "김삼",
    score: 76,
    city: "대구",
  },
  {
    name: "김사",
    score: 100,
    city: "부산",
  },
];
console.log(persons);

let pass = persons.filter(function (person) {
  return person.score > 80;
});

console.log(pass);

// map()
let userList = [
  {
    firstName: "일",
    lastName: "김",
    email: "1@test.com",
  },
  {
    firstName: "이",
    lastName: "김",
    email: "2@test.com",
  },
  {
    firstName: "삼",
    lastName: "김",
    email: "3@test.com",
  },
  {
    firstName: "사",
    lastName: "김",
    email: "4@test.com",
  },
  {
    firstName: "오",
    lastName: "김",
    email: "5@test.com",
  },
];

let koreanUserList = userList.map(function (user) {
  return {
    fullName: user.lastName.concat(user.firstName),
    firstName: user.firstName,
    lastName: user.lastName,
    email: user.email,
  };
});
console.log(koreanUserList);

// reduce()
let scores = [40, 100, 1, 5, 25, 10];
let sum = scores.reduce(function (total, currentValue) {
  return total + currentValue;
});
console.log(sum);
